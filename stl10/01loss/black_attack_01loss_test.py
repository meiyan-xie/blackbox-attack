...
class Target(object):
    ...
    def predict(self, x_sub):
        ...
        predicted_y = subprocess.check_output(['perl', '/research/datasci/mx42/stl10-original/01loss_test/predict_01loss.pl', 'test_sub', '/research/datasci/mx42/stl10-original/01loss_test/mapping_test'])
...
def get_data():
    ...
    testdata = testdataset[:30, 1:]
    testlabel = testdataset[:30, :1]
    print('testlabel0: ', testlabel)
    ...
    sub_x = testdata_tensor[indices[:20]].float().reshape((-1, 3, 96, 96))
    sub_x /= 255

    # test_data = testdata_tensor[indices[20:]].float().reshape((-1, 3, 96, 96))
    test_data = testdata_tensor.float().reshape((-1, 3, 96, 96))
    test_data /= 255

    # test_label = testlabel_tensor[indices[20:]].long()
    test_label = testlabel_tensor.long()
...
def stl10_bbox_sub(param, target_model, substitute_model, x_sub, test_data, \
    ...
    for rho in range(aug_epoch):
        ...
        if rho % 2 == 0:
            ...
            torch.save(substitute_model.model.state_dict(), 'model/sub_01loss.t7')
