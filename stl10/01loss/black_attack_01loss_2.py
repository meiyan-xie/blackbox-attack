...
class Target(object):
    ...
    def predict(self, x_sub):
        ...
        predicted_y = subprocess.check_output(['perl', '/research/datasci/mx42/stl10-original/01loss_inc50/predict_01loss.pl', 'test_sub', '/research/datasci/mx42/stl10-original/01loss_inc50/mapping_test'])
...
def get_data():
    test_dir = '/research/datasci/mx42/stl10-original/01loss_small_adv_sample_every2epoch/'
...
def jacobian_augmentation(model, x_sub, y_sub, Lambda, samples_max):
    ...
    x_sub = x_sub[indices[:50]]
    # print('x_sub 2: ', len(x_sub))

    if x_sub.size(0) <= samples_max:
...
def stl10_bbox_sub(param, target_model, substitute_model, x_sub, test_data, \
    ...
    for rho in range(aug_epoch):
        ...
        # Compute the accuracy adversarial samples on target model
            ...
            torch.save(substitute_model.model.state_dict(), 'model/sub_01loss_2200advSample.t7')
