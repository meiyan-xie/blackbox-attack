...
from models import LeNet, LinearModel, CNNModel
...
class Target(object):
    ...
    def predict(self, x_sub):
        ...
        # cmd = perl predict_bootstrap.pl cifar_testdata mapping_test
        predicted_y = subprocess.check_output(['perl', '/research/datasci/mx42/attack_cifar10/01loss_10majvote/predict_bootstrap.pl', 'test_sub', '/research/datasci/mx42/attack_cifar10/01loss_10majvote/mapping_test'])
        ...
        # Convert numpy ndarray to tensor (long type)
...
def get_data():
    data_dir = '/research/datasci/mx42/attack_cifar10/01loss_10majvote/'
    ...
    # ######## Save test_data for ploting image, just for debugging  #########
    ...
    # np.savetxt('test_data_bootstrap', test_data_np, fmt='%d')
    ...
    # np.savetxt('test_label_bootstrap', test_label_np, fmt='%d')
    # #################
...
def MNIST_bbox_sub(param, target_model, substitute_model, x_sub, test_data, \
    ...
    for rho in range(aug_epoch):
        ...
        # Compute the target model on the adv sample's accuracy
        n_sample = test_adv.size(0)

        # Saving the adversarial example at the last epoch
        ...
        #     np.savetxt('test_adv_bootstrap', test_adv_np, fmt='%d')
            ######################
        if (rho > 19 and rho % 2 == 0) or rho == aug_epoch - 1:
            ...
            torch.save(substitute_model.model.state_dict(), 'model/bootstrapsub10.t7')
