...
class Target(object):
    ...
    def predict(self, x_sub):
        ...
        predicted_y = subprocess.check_output(['perl', '/research/datasci/mx42/01loss/01loss_100majvote/predict_01loss.pl', 'test_sub', '/research/datasci/mx42/01loss/01loss_100majvote/mapping_test'])
...
def get_data():
    data_dir = '/research/datasci/mx42/01loss/01loss_100majvote/'
    ...
    np.savetxt('test_data_01loss100', test_data_np, fmt='%d')
    ...
    np.savetxt('test_label_01loss100', test_label_np, fmt='%d')
    #################
...
def MNIST_bbox_sub(param, target_model, substitute_model, x_sub, test_data, \
    ...
    for rho in range(aug_epoch):
        ...
        if rho == aug_epoch - 1:
            ...
            np.savetxt('test_adv_01loss100', test_adv_np, fmt='%d')
            ######################
        ...
        if rho % 2 == 0:
            ...
            torch.save(substitute_model.model.state_dict(), 'model/01loss_sub.t7')
