...
from models import LeNet, LinearModel
...
class Target(object):
    ...
    def predict(self, x_sub):
        ...
        np.savetxt('test_sub_svm', x_sub_np, fmt='%d')

        # Call svm predict
        predicted_y = svm_predict.main('test_sub_svm')
...
def get_data():
    data_dir = '/research/datasci/mx42/attack_cifar10/libsvm_10majvote/'
    ...
    # np.savetxt('test_data_svm100', test_data_np, fmt='%d')
    ...
    # np.savetxt('test_label_svm100', test_label_np, fmt='%d')
    # ################
...
def MNIST_bbox_sub(param, target_model, substitute_model, x_sub, test_data, \
    ...
    for rho in range(aug_epoch):
        ...
        # Eval substitute model on test data
        ...
        #     np.savetxt('test_adv_svm100', test_adv_np, fmt='%d')
        if rho % 2 == 0 or rho == aug_epoch - 1:
            ...
            torch.save(substitute_model.model.state_dict(), 'model/sub_svm10.t7')
